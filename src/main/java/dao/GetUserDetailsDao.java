...
public class GetUserDetailsDao {
	...
	public ArrayList<GetUserDetailsBean> getDeletedUserDetails()
	{
		ArrayList<GetUserDetailsBean> userDetails = new ArrayList<GetUserDetailsBean>();
		try
		{
			...
			con = DBConnection.DBCon();
			int size = 0 ;
			ps = con.prepareStatement("SELECT COUNT(*) FROM users WHERE Status = 'Deleted'");
			rs = ps.executeQuery();
			...
			ps = con.prepareStatement("SELECT Name, Email, Status FROM users WHERE Status = 'Deleted'");
			rs = ps.executeQuery();
